...
class Level:
	...
	def setup(self, tmx_map, level_frames, audio_files):
		...
		for obj in tmx_map.get_layer_by_name('Objects'):
			if obj.name == 'player':
				self.player = Player(
					# posisi player berada di tengan
		...
		for obj in tmx_map.get_layer_by_name('Enemies'):
			if obj.name == 'zombies':
				Zombies((obj.x, obj.y), level_frames['zombies'], (self.all_sprites, self.damage_sprites, self.zombies_sprites), self.collision_sprites)
			if obj.name == 'wizard':
				Wizard(
					pos = (obj.x, obj.y), 
	...
	def create_pearl(self, pos, direction):
		Pearl(pos, (self.all_sprites, self.damage_sprites, self.pearl_sprites), self.pearl_surf, direction, 250)
	...
	def attack_collision(self):
		targets = self.pearl_sprites.sprites() + self.zombies_sprites.sprites()
		
		for target in targets:
			is_target_right = self.player.rect.centerx < target.rect.centerx
		
			facing_target = (is_target_right and self.player.facing_right) or \
						(not is_target_right and not self.player.facing_right)
